            ...
            if (game_data.screen == 'am_farm') {
                $.when(buildOptions()).then((html) => {
                    ...
                    $('.optionButton').off('click').on('click', () => {
                        ...
                        html += `<br><input type="button" class="btn sendAllButton" value="Send all A & B"></div>`;
                    ...
                    $(document).on('click', '.sendAllButton', function() {
...
<br><input type="button" class="btn sendAllButton" value="Send all A & B">`;
    ...
    const sendAllFarms = function () {
        farmCount = 0;
        totalFarms = $('.farmGod_icon').length;
    ...
    const sendFarm = function ($this) {
        ...
        setTimeout(() => {
            ...
            TribalWars.post(Accountmanager.send_units_link.replace(/village=(\d+)/, 'village=' + $this.data('origin')), null, {
                ...
                UI.updateProgressBar($pb, farmCount, totalFarms);
